            ...
            <table class="col-12">
                ...
                @foreach($perfumes as $perfume)
                    <tr>
                        <td>{{$perfume->id}}</td>
                        <td>{{$perfume->nome}}</td>
                        <td>{{$perfume->fragrancia}}</td>
                        <td>{{$perfume->percent_agua}}</td>
                        <td>{{$perfume->percent_alcool}}</td>
                        <td>{{$perfume->percent_fragrancia}}</td>
                        <td>{{$perfume->total_produzido}}</td>
                        <td>{{$perfume->preco}}</td>
                        <td><a type="submit" href="{{route('editar_perfume',['id'=>$perfume->id])}}" class="btn btn-warning" 
                            title="Editar perfume {{$perfume->nome}}">Editar</a></td>
                        <td><a type="submit" href="{{route('excluir_perfume',['id'=>$perfume->id])}}" class="btn btn-danger" 
                            title="Excluir perfume {{$perfume->nome}}">Excluir</a></td>
                    </tr>

                @endforeach
            </table>
            <a tupe="submit" href="{{route('novo_perfume')}}" class="btn btn-success mb-5" 
                            title="Novo perfume ">Novo</a>
            </br>
        </div>
    </div> 
